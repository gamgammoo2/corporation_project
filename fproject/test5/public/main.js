...
async function toggle1() {
    ...
    const Div = document.getElementById('token_check');
...
/* 팝업창2 호출 이벤트 함수 정의 */
function call_popup2(up,down) {
    /* display 값 확인 */
    var tagId = document.getElementById("popup_container2");
    var display = window.getComputedStyle(tagId).display;
...
/* 팝업창3 호출 이벤트 함수 정의 */
function call_popup3(up,down) {
    /* display 값 확인 */
    var tagId = document.getElementById("popup_container3");
    var display = window.getComputedStyle(tagId).display;
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == XMLHttpRequest.DONE) {
                if (xhr.status == 200) {
                    ...
                    const errorResponse = JSON.parse(xhr.responseText);
                    const errorMessage = errorResponse.error;
                    call_popup('Warning!',errorMessage) //이미 있는 유저가 가입하려할때
    ...
    if (nickdouble1[0]=="" && nickdouble2[0]== "") {
        ...
        if (password==password2 ) { //다 만족
            if ( states[0][0].state=='correct') { //preuser
                ...
                const successmessage = "You are signed up.Try login now";
